...
class Vehicle {
    ...
    public:
        ...
        Vehicle(float speed) :startingSpeed(speed), aktualSpeed(speed), bremsweg(0.0f) {
           animationRunning = false;
           vehicleTexture.loadFromFile("Auto.png");
        ...
        void updateProperties() {
            properties["Bremsweg"] = std::to_string(bremswegInsgesamt);
            properties["AktuelleGeschwindigkeit"] = std::to_string(aktualSpeed);
            properties["StartGeschwindigkeit"] = std::to_string(startingSpeed);
        }
